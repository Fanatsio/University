function montage_new(I, titles, varargin)
...
for a = 1:layout(1)
    for b = 1:layout(2)
        ...
        duh(b) = subplot('Position', [(b-1)*hw(2)+gap(2) (a-1)*hw(1)+gap(1) dims]);
        ...
        if isempty(A)
            ...
            if(nargin>=2 && ~isempty(titles))
              title(titles{c}, 'FontSize', 15);
              
              set(get(duh(b), 'Title'), 'VerticalAlignment', 'baseline');
              set(get(duh(b), 'XLabel'), 'VerticalAlignment', 'baseline');              
            end
        end
    end
end
